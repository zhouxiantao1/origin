			...
			<div class="middle middle_1200 fl">
				<div class="nav_list clearfix">
					...
					<div class="new_zhibiao fr">
						<a onClick="javascript:window.location.href='${basePath!''}/base/student/create.htm'"> <img src="${basePath!''}/images/形状-4.png"> <i>新增人员管理</i></a>
					</div>
				</div>
				...
				<div class="all_content">
					<div class="operation clearfix">
						...
						<p id="student"  class="active">学生管理</p>
						<p id="parent" onClick="javascript:window.location.href='${basePath!''}/base/parent/list.htm'">家长管理</p>
						<!-- <div class="mingti fr">
							<button onClick="javascript:window.location.href='${basePath!''}/base/student/create.htm'"> <i>+</i> 新增</button>
						</div> -->
					</div>
					<div class="klass_sub">
						
						<span class="work_name">学生姓名：</span>
					...
					<#if pageBean.list?? && pageBean.list?size!=0>
						<#list pageBean.list as baseStudentVo>
							<div class="staff_list_contain">
			
								<div class="some_detail clearfix">
									<p class="fl staff_towe">${baseStudentVo.name!''}</p>
									<p class="fl staff_towe" ><#if baseStudentVo.sex=='1'>男<#elseif baseStudentVo.sex=='2'>女<#else>-</#if></p>
									<p class="fl staff_towe"><#if baseStudentVo.baseSchoolVo??>${baseStudentVo.baseSchoolVo.name!''}<#else>-</#if></p>
									<p class="fl staff_towe"><#if baseStudentVo.baseClassesVo??>${baseStudentVo.baseClassesVo.name!''}<#else>-</#if></p>
									<p class="fl staff_three"><span onClick="go_view(${baseStudentVo.id!''})">查看</span></p>
...
<script type="text/javascript">
	$(function(){
		<#if search??>
			$('#studentName').val('${search.name!''}')
		</#if>
	})
	
	function go_view(id){
		window.location.href="${basePath!''}/base/student/view.htm?id="+id;
	}
	...
	function search_ask(type,id){
		...
		var url = "${basePath!''}/base/student/list.htm?";
		if(name !=null && name != ''){
			data += "&name="+name;
		}
		if(type == 'page'){
