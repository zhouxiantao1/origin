package com.sjq.year.dao;

import java.util.List;
import java.util.Map;
import com.sjq.year.vo.BaseYearVo;
import com.sjq.year.bean.BaseYearBean;

/**
 * [自动生成]抽象基础模块-年份信息Dao层
 * @author  ShuiJingQiu
 * @since   2017-04-25 14:29:37
 * @version 1.0
 */
public interface BaseYearMapper {


	/**
	 * 查询所有的基础模块-年份信息信息
	 * @return 查询到的列表
	 */
	List<BaseYearVo> queryAllBaseYear();
	
	/**
	 * 根据ID查询基础模块-年份信息信息
	 * @param id 标识
	 * @return   查询到的列表
	 */
	BaseYearVo queryBaseYearById(Long id);
	
	/**
	 * 根据IDS查询所有基础模块-年份信息信息
	 * @param map 封装查询参数 
	 * @return    查询到的列表
	 */
	List<BaseYearVo> queryBaseYearByIds(Map<String, Object> map);	
			
	/**
	 * 根据条件查询所有基础模块-年份信息信息
	 * @param map 封装查询参数 
	 * @return    查询到的列表
	 */
	List<BaseYearVo> queryBaseYearBySearch(Map<String, Object> map);
	
	/**
	 * 分页查询基础模块-年份信息
	 * @param map 封装查询参数 
	 * @return    查询到的列表
	 */
	List<Object> queryBaseYearPage(Map<String, Object> map);
	
	/**
	 * (删除)根据ID删除基础模块-年份信息信息
	 * @param id 标识
	 * @return   受删除影响的行数
	 */
	int deleteBaseYearById(BaseYearBean bean);
	
	/**
	 * (删除)根据IDS删除基础模块-年份信息信息
	 * @param map 封装查询参数 
	 * @return    受删除影响的行数
	 */
	int deleteBaseYearByIds(Map<String, Object> map);
		
	/**
	 * (更新)根据ID更新所有基础模块-年份信息信息
	 * @param bean 基础模块-年份信息实体
	 * @return     受更新影响的行数
	 */
	int updateBaseYearById(BaseYearBean bean);
	
	/**
	 * 根据条件查询基础模块-年份信息count总数
	 * @param map 封装查询参数 
	 * @return    查询到符合条件的总数
	 */
	int queryBaseYearTotalCount(Map<String, Object> map);
	
	
}