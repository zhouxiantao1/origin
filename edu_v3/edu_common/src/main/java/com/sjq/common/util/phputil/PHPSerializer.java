...
public class PHPSerializer {
    ...
    private static int writeObject(ByteArrayOutputStream stream, Object obj, HashMap ht, int hv, String charset) {
        ...
        if (obj instanceof java.io.Serializable) {
            ...
            if (obj instanceof com.sjq.common.util.phputil.Serializable) {
                byte[] cs = ((com.sjq.common.util.phputil.Serializable) obj).serialize();
    ...
    private static UnSerializeResult readCustomObject(ByteArrayInputStream stream, HashMap ht, int hv, String charset) {
        ...
        if (o == null) {
            stream.skip(n);
        } else if (o instanceof com.sjq.common.util.phputil.Serializable) {
            byte[] b = new byte[n];

            stream.read(b, 0, n);
            ((com.sjq.common.util.phputil.Serializable) o).unserialize(b);
        } else {
            stream.skip(n);
        }
        stream.skip(1);
